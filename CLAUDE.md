...
- `npm run test:coverage` — runs `c8 jest`. Uses V8 native coverage (not Istanbul) so spawned subprocesses from the integration suite are instrumented too; `bin/cli.js` and `lib/commands/*` show real numbers. Config is `.c8rc.json`. Writes `coverage/` (gitignored). CI runs this on the Node 20 matrix job so the text table prints in the workflow log; the HTML/lcov artifacts are not uploaded — run locally if you need the rendered report.
